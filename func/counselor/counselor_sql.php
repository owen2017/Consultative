<?php
	...
	class counselor_sql{
	    ...
	    # 申請列表 - 新增
	    static function sql_ins_counselor_apply($_account,$_password,$_name,$_gender,$_identity,$_mobile,$_phone,$_email,$_area,$_office_time,$_office_area,$_job,$_service_area,$_serviceobj,$_serviceLimit,$_charges,$_seniority,$_training,$_experience,$_case_times,$_education,$_license,$_license_num,$_specialty,$_idea1,$_idea2){
	...
	# 取得列表
	    static function sql_front_get_list($_acc, $_name, $_area, $_satisfaction, $_specialty, $_page, $_rows){
	    	...
	    	while ($row = mysqli_fetch_array($result, MYSQL_ASSOC)) {
	    		...
	    		// $row["c_specialty"] = $de_specialty["checkbox"];
	    		// $row["g_specialty"] = $de_specialty["general"];
	    		// $row["area"] = AREA::get_str($row["area"]);
	    		$row["area"] = $row["area"];
				$ary[] = $row;
